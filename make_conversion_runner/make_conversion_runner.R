...
template = "dx run app-swiss-army-knife -iin='FILE_ID' -icmd='bcftools norm -m -both FILE_NAME.vcf.gz | bgzip -c > normalized_FILE_NAME.vcf.gz; plink2 --vcf normalized_FILE_NAME.vcf.gz -vcf-half-call m --vcf-min-dp 10 --make-bed --out FILE_NAME;' --destination '/plink_files' --name FILE_NAME --instance-type mem2_ssd1_v2_x8 --brief -y;"
